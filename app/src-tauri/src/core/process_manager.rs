//! Cross-platform PTY session manager.
//!
//! Originally from https://github.com/its-maestro-baby/maestro
//! commit a10500d, MIT licensed by Maestro contributors.
//! Adapted for gmux-v4 by fivelidz:
//!   - renamed env vars: MAESTRO_SESSION_ID → GMUX_SESSION_ID
//!                       MAESTRO_PROJECT_HASH → GMUX_PROJECT_HASH
...
impl ProcessManager {
    ...
    /// - `GMUX_SESSION_ID` is automatically set to the session ID
    /// - Additional env vars can be passed via the `env` parameter (e.g., `GMUX_PROJECT_HASH`)
    ///
    /// # Windows Debouncing
    ...
    pub fn spawn_shell(
        ...
        // Inject GMUX_SESSION_ID automatically (used by MCP status server)
        cmd.env("GMUX_SESSION_ID", id.to_string());
